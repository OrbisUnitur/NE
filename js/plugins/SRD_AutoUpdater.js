 ...
 *
 *   /img/NeUpd/auto-updates/Background.png
 *
...
Imported["NeUpd Auto Updater"] = 1.00;
...
if(_.url.trim().length === 0) alert("Yo! You don't have an 'Update Text URL' set up. Prepare to have your game crash!");
...
_.downloadUpdateFile = function(url) {
	...
	_.download(url, _.localPath('save/updates.NeUpd'), function() {
		_.loadUpdateFile();
	})
};
...
_.loadUpdateFile = function() {
	var fs = require('fs');
	fs.readFile(_.localPath('save/updates.NeUpd'), 'utf8', function(err, contents) {
...
_.deleteUpdateFile = function() {
	var fs = require('fs');
	fs.writeFile(_.localPath('save/updates.NeUpd'), '', function(err) {
		if (err) throw err;
	})
};
...
_.localFilePathVersion = function() {
	return StorageManager.localFileDirectoryPath() + 'version.NeUpd';
};
...
_.loadImage = function(filename, hue) {
	return ImageManager.loadBitmap('img/UpDat/auto-update/', filename, hue, true);
};
...
Scene_AutoUpdate.prototype.setupText = function() {
	...
	this._text1._myText = 'Please Wait';
...
Scene_AutoUpdate.prototype.updateImages = function() {
	if(this._parallax && this._blackPic) {
		...
		if(Math.abs(this._text1.rotation) > 0.0) this._text1._rotationSpeed *= (-1);
		if(this._text1._dotDotDotSpeed < 45) {
			this._text1._dotDotDotSpeed++;
		} else {
			if(this._text1._myText === 'Please Wait...') {
				this._text1._myText = 'Please Wait';
			} else {
